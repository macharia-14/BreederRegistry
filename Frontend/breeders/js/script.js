...
function initSidebar() {
  ...
  window.toggleMenu = () => setSidebarOpen(!sidebar?.classList.contains('active'));
  ...
  navItems.forEach(item => {
    item.classList.toggle('active', item.getAttribute('href') === fileName || (fileName === 'animal_profile.html' && item.getAttribute('href') === 'animal_management.html'));
...
async function loadDashboardStats() {
  ...
  const now = new Date();
  // Active pregnancies are explicit: confirmed/open records only, not guessed from missing offspring.
  const pregnancies = events.filter(ev => ev.status === 'confirmed_pregnant' && !ev.outcome).length;

  // Births this year are explicit live-birth outcomes.
  const birthsThisYear = events.filter(ev => {
    if (ev.outcome !== 'live_birth') return false;
    const date = ev.outcome_date ? new Date(ev.outcome_date) : new Date(ev.expected_due_date || ev.breeding_date);
    return date.getFullYear() === now.getFullYear();
  }).length;

  // Success rate is calculated from closed outcomes only.
  const closedEvents = events.filter(ev => ['live_birth','failed_conception','miscarriage','stillbirth','abortion'].includes(ev.outcome));
  const successfulEvents = closedEvents.filter(ev => ev.outcome === 'live_birth').length;
  const successRate = closedEvents.length > 0 ? ((successfulEvents / closedEvents.length) * 100).toFixed(0) + '%' : 'N/A';
...
async function loadOffspringHistoryPageData() {
  ...
  const successfulEvents = events.filter(ev => ev.offspring_id || ev.outcome === 'live_birth');
...
function generateLineageHtmlFromPostgres(lineageData) {
    ...
    // Horizontal connector between two parent groups
...
async function handleBreedingEvent(e) {
  ...
  try {
    const eventData = {
      ...
      status:            document.getElementById('offspring').value ? 'completed' : 'served',
      pregnancy_confirmed: Boolean(document.getElementById('offspring').value),
      outcome:           document.getElementById('offspring').value ? 'live_birth' : null,
      // Conditional fields
...
async function generateComprehensiveFarmReport() {
    ...
    try {
        ...
        const femaleCount = animals.filter(a => a.gender === 'female').length;
        const pregnancies = events.filter(ev => ev.status === 'confirmed_pregnant' && !ev.outcome).length;
        const completedEvents = events.filter(ev => ['live_birth','failed_conception','miscarriage','stillbirth','abortion'].includes(ev.outcome));
        const successfulEvents = completedEvents.filter(ev => ev.outcome === 'live_birth').length;
        ...
        const eventBody = events.map(ev => {
            let status = ev.status || 'In Progress';
            if (ev.outcome === 'live_birth') status = `Success (${ev.live_offspring_count ?? ev.offspring_count ?? 0} live)`;
            else if (ev.outcome) status = ev.outcome.replaceAll('_', ' ');
            else if (ev.expected_due_date && new Date(ev.expected_due_date) < now) status = 'Overdue / pending update';
            return [
...
async function generateBreedingPerformanceReport() {
    ...
    try {
        ...
        const methodStats = completedEvents.reduce((acc, ev) => {
            ...
            acc[method].total++;
            if (ev.outcome === 'live_birth') acc[method].success++;
            return acc;
        }, {});
        ...
        const sireStats = completedEvents.reduce((acc, ev) => {
            ...
            acc[sireId].total++;
            if (ev.outcome === 'live_birth') acc[sireId].success++;
            return acc;
        }, {});
        const animalIdMap = new Map(animals.map(a => [a.id, a.animal_id]));
...
async function handleBreedingEvent(e) {
 ...
 //breeder-side stabilization, reporting, settings,exports, and activity-log integration
...
// Handles export all animals csv behavior for this page.
async function exportAllAnimalsCsv() { return exportToCsv(); }

// Handles export all events csv behavior for this page.
async function exportAllEventsCsv() {
  const { animals, events } = await phase8LoadAnimalsEventsSummary();
  if (!events.length) return showToast('No breeding events to export.', 'warning');
  const map = new Map(animals.map(a => [a.id, a.animal_id]));
  phase8DownloadCsv(`breeding_events_${new Date().toISOString().slice(0,10)}.csv`,
    ...
    events.map(e => [e.id, e.breeding_date, e.expected_due_date, map.get(e.dam_id) || e.dam_id, e.sire_id ? map.get(e.sire_id) || e.sire_id : '', e.breeding_method, e.status, e.outcome, e.pregnancy_confirmed, e.offspring_count, e.live_offspring_count, e.notes || e.outcome_notes || ''])
  );
}
...
// Loads reports page data from the API or page state.
async function loadReportsPage() {
  const box = phase8EnsureCard('reportsSummaryPanel', 'Live Report Summary');
  const data = await apiFetch(`/api/breeders/${breederId}/report-summary`, { headers: { Authorization: `Bearer ${getToken()}` } });
  const s = data.summary || {};
  box.innerHTML = `<div class="stats-grid">
    <div class="stat-card"><div class="stat-value">${s.total_animals || 0}</div><div class="stat-label">Animals</div></div>
    <div class="stat-card"><div class="stat-value">${s.active_pregnancies || 0}</div><div class="stat-label">Active Pregnancies</div></div>
    <div class="stat-card"><div class="stat-value">${s.pending_pregnancy_checks || 0}</div><div class="stat-label">Checks Due</div></div>
    <div class="stat-card"><div class="stat-value">${data.breeding_analytics?.summary?.success_rate ?? '—'}${data.breeding_analytics?.summary?.success_rate == null ? '' : '%'}</div><div class="stat-label">Closed Success Rate</div></div>
  </div>
  <div class="info-card" style="margin-top:1rem;"><h3>Top Breeds</h3><p>${Object.entries(data.animal_breakdown?.by_breed || {}).map(([k,v]) => `${k}: ${v}`).join(' · ') || 'No breed data yet.'}</p></div>`;
  document.getElementById('generateFarmReportBtn')?.addEventListener('click', generateComprehensiveFarmReport);
  document.getElementById('generateBreedingReportBtn')?.addEventListener('click', generateBreedingPerformanceReport);
  document.getElementById('exportAllAnimalsCsvBtn')?.addEventListener('click', exportAllAnimalsCsv);
  document.getElementById('exportAllEventsCsvBtn')?.addEventListener('click', exportAllEventsCsv);
...
// Override Animal Management table rendering with a cleaner action order and profile-first detail view.
...
// Run after the original boot logic creates breederId and loads breederData.
document.addEventListener('DOMContentLoaded', () => {
  setTimeout(async () => {
...
// overview intelligence, and offspring register reliability
...
function phase10EnhanceHeader() {
  ...
  // Keep farm selector and bell on the same row, in the order: Farm selector → Bell.
...
document.addEventListener('DOMContentLoaded', () => {
 ...
 //counts delivery/birth outcomes even when the breeder has recorded a delivery event but has not yet registered each newborn animal individually.
...
// Fixes: missing offspring PDF export, header alignment after inline CSS,
// reports sidebar icon consistency, genetics header styling, and breeding
...
// BREEDER MOBILE SIDEBAR STANDARDIZATION
