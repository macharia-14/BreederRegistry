// Check for URL parameters on load
      document.addEventListener('DOMContentLoaded', () => {
        ...
        // ?animal=PBK-001  →  auto-run lineage search
        ...
        // ?breed=Angus  →  auto-run breed/breeder search (legacy param)
      ...
      async function handleLineageSearch(e) {
        ...
        // Check if input looks like animal ID (e.g. PBK-001) or breed (text)
      ...
      async function performLineageSearch(animalId) {
        ...
        try {
          ...
          // Extract data from the success envelope { success: true, data: [...] }
          const data = json.data ?? json;
          if (!data || (Array.isArray(data) && data.length === 0)) {
            throw new Error("Not found");
          }

          // Pass the root animal (generation 0) to the renderer
      ...
      function renderResult(a) {
        // The backend returns a list where parents are IDs.
        const sireId = a.sire_id || null;
        const damId = a.dam_id || null;

        // Note: For a full tree, you'd typically use the generation data
        // returned in the list. This implementation uses a simplified node view.

        /*
        // Grandparents (if nested in API response)
        const patGrandsire = sire?.sire || null;
        const patGranddam = sire?.dam  || null;
        const matGrandsire = dam?.sire  || null;
        const matGranddam = dam?.dam   || null;
        ...
        */
        ...
        // Draw SVG connector lines after render
      ...
      function drawConnectors() {
        ...
        if (parentRow && connSvg) {
          ...
          if (nodes.length === 2) {
            ...
            // Horizontal bar at top + drop to center
            paths = `
